...
<table width="100%" border="0">
  <tr>
	...
	<tr align="center">
		<td>เปลี่ยนข้อความ และ สีพื้นหลัง</td>
		</tr>
	<tr>
		<td>
			<table align="center">
			<tr><td><form name="form1" method="POST" action="admin_cafe_text_save.php?ctext_id=<?php echo $ctext_id ?>" enctype="multipart/form-data">
					<?php
					$result=mysql_query("SELECT * from cafe_text where ctext_id='$ctext_id'");
					$row=mysql_fetch_assoc($result);
...
?>
					...
					<tr>
							...
							<span class="input-group-addon">ข้อความ</span>
								<textarea type="text" class="form-control" placeholder="" name="ctext_info" id="ctext_info" rows="10"><?php echo $ctext_info ?></textarea>
							</div>

							<div class="input-group">
						<span class="input-group-addon">สีตัวอักษร</span>
							<input type="color" class="form-control" placeholder="" name="ctext_font" id="ctext_font" value="<?php echo $ctext_font ?>">
						</div>

						<div class="input-group">
					<span class="input-group-addon">สีพื้นหลัง</span>
						<input type="color" class="form-control" placeholder="" name="ctext_bg" id="ctext_bg" value="<?php echo $ctext_bg ?>">
					</div>
							</td>
						</tr>
							<tr>
