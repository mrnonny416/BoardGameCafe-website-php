<?php
    ...
    if (isset($_GET["cphoto_id"])) {
        $cphoto_id = $_GET["cphoto_id"];
    } else {
        // กำหนดค่าเริ่มต้นให้กับ $cphoto_id หากไม่ได้รับค่าผ่าน URL
        $cphoto_id = 0;
    }
                    ...
                    <tr>
                        <td>
                            <form name="form1" method="POST"
                                action="admin_cafe_photo_save.php?cphoto_id=<?php echo"$cphoto_id"?>"
                                enctype="multipart/form-data">
                                <?php
					$result=mysqli_query($conn,"SELECT * from cafe_photo where cphoto_id=$cphoto_id");
					$row=mysqli_fetch_assoc($result);
